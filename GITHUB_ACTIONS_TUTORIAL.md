...
팀 상황에 따라 두 가지 전략 중 선택할 수 있다.

#### 전략 1: Push + PR 둘 다 CI 실행
...
```
commit → push → CI 실행 → PR 생성 → CI 다시 실행 → merge
                  ↑                      ↑
               빠른 피드백            최종 검증
```
...
| 장점 | 단점 |
|------|------|
| push 직후 바로 피드백 | CI 리소스 더 많이 사용 |
| PR 전에 미리 문제 발견 | 작은 커밋마다 CI 실행 |
| 개발자가 빠르게 수정 가능 | GitHub Actions 무료 시간 소모 |

**적합한 경우:**
- 팀원이 자주 커밋하고 빠른 피드백 원할 때
- CI 실행 시간이 짧을 때 (5분 이내)
- GitHub Actions 무료 시간이 충분할 때
...
```
commit → push → (CI 안 돔) → PR 생성 → CI 실행 → merge
                                          ↑
                                      이때만 검증
```
...
| 장점 | 단점 |
|------|------|
| CI 리소스 절약 | PR 올리기 전까지 문제 모름 |
| 불필요한 CI 실행 방지 | 피드백 늦음 |
| 무료 시간 아낄 수 있음 | PR에서 실패하면 다시 수정 필요 |

**적합한 경우:**
- CI 실행 시간이 길 때 (10분 이상)
- 무료 시간 제한이 있을 때
- 개발자가 로컬에서 충분히 테스트 후 push할 때
...
│     NO  → 전략 1로 빠른 피드백 필요                      │
├─────────────────────────────────────────────────────────┤
│  GitHub Actions 무료 시간 (월 2000분) 여유 있는가?        │
│     YES → 전략 1                                        │
│     NO  → 전략 2                                        │
└─────────────────────────────────────────────────────────┘
```
